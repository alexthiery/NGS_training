...
In this session, we will conduct bulk ATACseq analysis in R using [DESeq2](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-014-0550-8) for data normalisation and differential accessibility testing.
...
In the last session we worked with the aligned reads directly, however in ATAC-seq data we can often use just look at regions of the genome defined as 'peaks' instead of looking at all reads across all parts of the genome. 

Peaks are regions of the genome where lots of reads have mapped, they are identified by peak calling algorithms such as [Macs2](https://hbctraining.github.io/Intro-to-ChIPseq/lessons/05_peak_calling_macs.html). The [nf-core ATAC-seq pipeline](https://nf-co.re/atacseq/2.0/output) that was used to pre-process our ATAC-seq reads also ran Macs2 to call peaks on our samples. 

Peak calling can be performed on each sample individually or on all the samples and replicates together. The [MultiQC report](https://moodle.learninghub.kingshealthpartners.org/pluginfile.php/184577/mod_resource/content/3/multiqc_report.html#mlib_peak_count) includes plots from the Macs2 output showing us how many peaks were called on each sample and how these peaks are annotated. 

We are going to look at a consensus peak set that was called on our 6 samples (3 conditions, 2 replicates of each).
...
peaks_bed_path <- "./raw_data/Peak_files/consensus_peaks.bed"
...
In order to give our peak coordinates biological context, we need to know where they are relative to genes and other annotated features of the genome. 

Annotation of the peaks BED file can be done manually by intersecting two BED files: the peaks BED file and the genome annotations BED file. This can be done using a function called [bedtools intersect](https://bedtools.readthedocs.io/en/latest/content/tools/intersect.html). In our case, the NF-core pipeline already includes a step that annotated the peak bed file so we only have to read it in. 


```{r annotate_peak_bed}

annot_peaks_bed_path <- "./raw_data/Peak_files/consensus_peaks_annotated.bed"
...
peak_count_matrix_path <- "./raw_data/Peak_files/consensus_peaks_featureCounts.txt"
...
Next we set the reference level of the Group column to STD. Usually this would be set to your control sample, purely to make subsequent fold change calculations relative to your control.
```{r}
...
Here we filtering peaks which have fewer than 5 counts in at least 2 samples
```{r}
...
The standard differential accessibility analysis steps are wrapped into a single function, DESeq.
...
During this step, DESeq2 will:
  ...
  3) Fit generalised linear models of gene accessibility for each peak

```{r}
deseq <- DESeq(deseq)
```
Results tables of specific pairwise differential accessibility tests can then be retrieved using the *results* function. This extracts a results table with log2 fold changes, p-values and adjusted p-values.
...
annot_peaks_bed_temp <- annot_peaks_bed[, -1]
rownames(annot_peaks_bed_temp) <- annot_peaks_bed[, 1]
head(annot_peaks_bed_temp)

annot_peaks_bed_temp[rownames(res)[1], ]
