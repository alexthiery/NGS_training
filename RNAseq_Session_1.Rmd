...
You can check if an array contains only unique elements by returning a boolean array as to whether or not any elements are duplicated
...
For this we first want to set the gene names as the rownames of the read_counts object, we then want to remove the gene_id and gene_annotation columns from our dataframe as DESeq2 expects a dataframe containing only counts.
...
Here I am filtering genes which have fewer than 10 readcounts across all samples
```{r}
...
DESeq2 will run count normalisation using the medians of ratios method, which effectively corrects for differences in sequencing depth between samples.

```{r}
deseq <- DESeq(deseq)
```

The size factors shown below are the values for which gene expression in each sample is scaled for in order to account for sequencing depth and RNA composition. Further information as to why and how these size factors are calculated can be found [here](https://hbctraining.github.io/DGE_workshop/lessons/02_DGE_count_normalization.html).
```{r}
sizeFactors(deseq)
```
