...
```{r}
read_counts <- read.delim('./rnaseq_read_counts.tsv', stringsAsFactors = FALSE)
```
...
```{r}
deseq_temp <- deseq
deseq_temp$Group <- relevel(deseq_temp$Group, ref = "K562")
deseq_temp <- DESeq(deseq_temp)
resultsNames(deseq_temp)
res <- results(deseq_temp, name=c("Group_MCF7_vs_K562"))
